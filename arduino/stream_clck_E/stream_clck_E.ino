    ...
    if (Wire.available()==1)
      {
        ...
        s = bitRead(c,7); s = (s<<1)+bitRead(c,6);
          ...
          String D2_cal = "cal,e,"+String(m)+","+String(a)+","+String(g)+","+String(s);
          char mydata_cal[D2_cal.length()+1];
          D2_cal.toCharArray(mydata_cal,D2_cal.length()+1);
          RFduinoGZLL.sendToHost(mydata_cal,D2_cal.length()+1);
          delay(random(10,25));
          } 
  flg = 1;
  digitalWrite(3,HIGH);
...
String D2 = "e,"+String(Q0)+","+String(Q1)+","+String(Q2)+","+String(Q3);
char mydata[D2.length()+1];
D2.toCharArray(mydata,D2.length()+1);
RFduinoGZLL.sendToHost(mydata,D2.length()+1);
delay(random(10,25));
}
