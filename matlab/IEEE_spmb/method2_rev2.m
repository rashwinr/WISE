...
j = 1;
while j<jamx %rotation capturing

        q = ModReceive(s,WISEmod,q);
        q1 = quatmultiply(quatconj(qref),q);
        
        fq = quatmultiply(quatconj(Qref),q);
        absAx = quat2axang(fq);
        absTH = absAx(4);
...
disp('GO')
%% start the motor to check on Q1
pause(2)
...
disp('GO')
%% strat the motor to check on the trnsf
pause(2)
listQ1 = [];
