...
k=[];
figure(2)
title('Shoulder Flexion-Extension Left hand','FontWeight','bold','FontSize',font)
set( gcf, 'DoubleBuffer', 'on','keypress','k=get(gcf,''currentchar'');' );
hold on
...
hold off
figure(1)
[~,~] = system('F:\unityrecordings\Leftflexext.mp4');                                                                       %Left arm extension flexion                                                                                        
while lec && telapsed<=60
    tstart=tic;
    ...
    if validData
       ...
       if numBodies == 1
            ...
            leftElbowprojection = LH2 - dot(LH2,coronalnormalL)*coronalnormalL;A = cross(TrunkVector,LH2);
            lkinbdangle = sign(-A(3))*atan2d(norm(A),dot(TrunkVector,LH2));                                                 %Abduction-adduction angle left
            rightElbowprojection = RH2 - dot(RH2,coronalnormalR)*coronalnormalR;B = cross(TrunkVector,RH2);
...
close figure 2
telapsed = 0;lbd = 0;lbdc=1;lbdflag = 0;
figure(2)
title('Shoulder Abduction-Adduction Left hand','FontWeight','bold','FontSize',font)
set( gcf, 'DoubleBuffer', 'on','keypress','k=get(gcf,''currentchar'');' );
hold on
xlabel('Time (seconds)','FontWeight','bold','FontSize',font);
ylabel('Joint angles (degrees)','FontWeight','bold','FontSize',font);
...
hold off
figure(1)
[~,~] = system('F:\unityrecordings\leftabdadd.mp4');                                                                        %Left arm abduction-adduction
while lbdc && telapsed<=60
    tstart=tic;
    validData = k2.updateData;                                                                                              %Kinect section
    if validData
        depth = k2.getDepth;color = k2.getColor;face = k2.getFaces;
       ...
       if numBodies == 1
            ...
            if lkinbdangle>=160 && lbdflag && lbdc
                lbd=lbd+1;
                lbdflag =0;
                if lbd>=8
                    close figure 2
                    lbdc = 0;
                    clearvars lbdc lbdflag lbd
                    break;
                end
            end
            clearvars A B C D E F leftElbowprojection rightElbowprojection coronalnormalL coronalnormalR armaxisnormalL armaxisnormalR
...
text(ls+lw/3,1050,'Time (seconds)','Color','white','FontSize',fs/(fontdiv),'FontWeight','bold','HorizontalAlignment','center');
text(ls+(limulocationdiv*lw),1000,num2str(telapsed,'%.2f'),'Color','white','FontSize',fs/fontdiv,'FontWeight','normal','HorizontalAlignment','center');
addpoints(anline,telapsed,lkinbdangle);
drawnow
        end   
 if numBodies == 0
      s1 = strcat('No persons in view');   
...
close figure 2
telapsed = 0;leef = 0;leefc=1;leefflag = 0;
figure(2)
...
hold off
figure(1)
[~,~] = system('F:\unityrecordings\leftelbowflexext.mp4');                                                                  %Left elbow flexion-extension
while leefc && telapsed<=60
    tstart=tic;
    validData = k2.updateData;                                                                                              %Kinect section
    if validData
        depth = k2.getDepth;color = k2.getColor;face = k2.getFaces;
       ...
       if numBodies == 1
            ...
            if lkinelbangle>=140 && leefflag && leefc
                leef=leef+1;
                leefflag =0;
                if leef>=8
                    close figure 2
                    leefc = 0;
                    clearvars leef leeefc leefflag
                    break;
                end
            end
            clearvars A B C D E F leftElbowprojection rightElbowprojection coronalnormalL coronalnormalR armaxisnormalL armaxisnormalR
...
text(ls+lw/3,1050,'Time (seconds)','Color','white','FontSize',fs/(fontdiv),'FontWeight','bold','HorizontalAlignment','center');
text(ls+(limulocationdiv*lw),1000,num2str(telapsed,'%.2f'),'Color','white','FontSize',fs/fontdiv,'FontWeight','normal','HorizontalAlignment','center');
addpoints(anline,telapsed,lkinelbangle);
drawnow
        end   
 if numBodies == 0
      s1 = strcat('No persons in view');   
...
% hold off
% figure(1)
% [~,~] = system('F:\unityrecordings\leftelbowflexext.mp4');                                                                  %Left elbow flexion-extension with 90 degree abduction
% while leefc && telapsed<=60
%     tstart=tic;
%     validData = k2.updateData;                                                                                              %Kinect section
%     if validData
...
% end
close figure 2
telapsed = 0;lie = 0;liec=1;lieflag = 0;
figure(2)
title('Left elbow flexion-extension','FontWeight','bold','FontSize',font)
set( gcf, 'DoubleBuffer', 'on','keypress','k=get(gcf,''currentchar'');' );
hold on
...
hold off
figure(1)
[~,~] = system('F:\unityrecordings\leftintext.mp4');                                                                        %Left shoulder internal external rotation
while liec && telapsed<=60
    tstart=tic;
    validData = k2.updateData;                                                                                              %Kinect section
    if validData
        depth = k2.getDepth;color = k2.getColor;face = k2.getFaces;
       ...
       if numBodies == 1
            ...
            if lkinieangle>=140 && lieflag && liec
                lie=lie+1;
                lieflag =0;
                if lie>=8
                    close figure 2
                    liec = 0;
                    clearvars lie liec lieflag
                    break
                end
            end
            clearvars A B C D E F leftElbowprojection rightElbowprojection coronalnormalL coronalnormalR armaxisnormalL armaxisnormalR
...
text(ls+lw/3,1050,'Time (seconds)','Color','white','FontSize',fs/(fontdiv),'FontWeight','bold','HorizontalAlignment','center');
text(ls+(limulocationdiv*lw),1000,num2str(telapsed,'%.2f'),'Color','white','FontSize',fs/fontdiv,'FontWeight','normal','HorizontalAlignment','center');
addpoints(anline,telapsed,lkinieangle);
drawnow
        end   
 if numBodies == 0
      s1 = strcat('No persons in view');   
