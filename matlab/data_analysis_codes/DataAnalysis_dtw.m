    ...
    if f2(2) == "WISE+KINECT" && f2(1)==num2str(SID)
        ...
        switch(typ)
            ...
            case markers(1)
                ...
                hold on
                plot(Time,lfe(:,2),'b');
                plot(Time1,lfe(X,1),'r-.');
                plot(Time1,lfe(Y,2),'b-.');
                ...
                rmse1 = signal_RMSE(lfe(X,1),lfe(Y,2));
                rmse2 = signal_RMSE(lfe(:,1),lfe(:,2));
                err = abs(lfe(X,1)-lfe(Y,2));
                mean(err)
                std(err)
                figure(2)
                hold on
                ...
                [pkinect,kloc] = findpeaks(lfe(X,1),Time1,'MinPeakHeight',100,'MinPeakProminence',50,'NPeaks',7);
                [pwise,wloc] = findpeaks(lfe(Y,2),Time1,'MinPeakHeight',100,'MinPeakProminence',50,'NPeaks',7);
